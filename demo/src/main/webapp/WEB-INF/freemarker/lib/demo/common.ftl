...
<#macro left selected>
    ...
    <div class="st_tree" style="overflow-y:auto;">
        <ul class="site-stats">
            <li <#if selected == "QCFXPJ">class="active"</#if>><a href="<@tags.spring.url value='/smkj/qcfxpj' />" onclick="javascript:$('body').showLoading();" >汽车风险评级</a></li>
            <li <#if selected == "JSXWFX">class="active"</#if>><a href="<@tags.spring.url value='/smkj/jsxwfx/YYZX03810' />" onclick="javascript:$('body').showLoading();" >驾驶行为分析</a></li>
            <li <#if selected == "BXDJYZ">class="active"</#if>><a href="<@tags.spring.url value='/bxdjyz/bxdjyz.xlsx' />" >保险定价因子</a></li>
            <li <#if selected == "QCSYKSH">class="active"</#if>><a href="<@tags.spring.url value='/smkj/qcsyksh' />" onclick="javascript:$('body').showLoading();" >汽车使用可视化</a></li>
        </ul>
    </div>
    <!--left nav end-->
</#macro>
